"""Citi Bike destination crime analysis: do rides ending in high-crime zones have
higher excess duration than rides ending in low-crime zones?

Treatment: end station zone crime_decile >= HIGH_CRIME_MIN_DECILE (top 30%).
Control:   end station zone crime_decile <= CONTROL_MAX_DECILE (bottom 50%).
...
taxi_zones.geojson to attach end_crime_decile.
"""
...
def main() -> None:
    ...
    # 2. Load trips from passthrough output (already has pct_time_increase, end_zone_id)
    trips = pd.read_parquet(f"{INPUT_DIR}/citibike_passthrough_analysis.parquet")
    trips = trips.dropna(subset=["pct_time_increase", "end_zone_id"]).copy()
    trips["end_zone_id"]      = trips["end_zone_id"].astype(int)
    trips["end_crime_decile"] = trips["end_zone_id"].map(zone_decile_map)
    trips = trips.dropna(subset=["end_crime_decile"]).copy()
    trips["end_crime_decile"] = trips["end_crime_decile"].astype(int)
    print(f"Total trips: {len(trips):,}")

    g0 = trips[trips["end_crime_decile"] <= CONTROL_MAX_DECILE]["pct_time_increase"]
    g1 = trips[trips["end_crime_decile"] >= HIGH_CRIME_MIN_DECILE]["pct_time_increase"]
    n_mid = len(trips) - len(g0) - len(g1)

    print(f"\nControl (end decile ≤ {CONTROL_MAX_DECILE}):                n={len(g0):,}")
    print(f"Middle  (decile {CONTROL_MAX_DECILE+1}–{HIGH_CRIME_MIN_DECILE-1}, excluded):       n={n_mid:,}")
    print(f"Treated (end decile ≥ {HIGH_CRIME_MIN_DECILE}):                n={len(g1):,}")
    ...
    trips.to_parquet(f"{OUTPUT_DIR}/citibike_end_crime_analysis.parquet", index=False)

    # --- Bar chart ---
    fig, ax = plt.subplots(figsize=(7, 5))
    labels = [
        f"Low-crime end zone\n(decile ≤ {CONTROL_MAX_DECILE},  n={len(g0):,})",
        f"High-crime end zone\n(decile ≥ {HIGH_CRIME_MIN_DECILE},  n={len(g1):,})",
    ]
    ...
    ax.set_title(
        "End-Zone Crime Level and Trip Duration Excess — Citi Bike\n"
    ...
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/citibike_end_comparison.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"\nSaved citibike_end_comparison.png")

    # --- Binned scatter ---
    grouped  = trips.groupby("end_crime_decile")["pct_time_increase"]
    deciles  = sorted(trips["end_crime_decile"].unique())
    ...
    ax.set_xlabel("End Station Zone Crime Decile  (1 = Lowest, 10 = Highest)", fontsize=11)
    ax.set_ylabel("% Trip Time Increase  (actual / OSRM − 1) × 100", fontsize=11)
    ax.set_title(
        "Route Detour by End-Zone Crime Level — Citi Bike\n"
        "Weekday Peak-Hour Members,  NYC Jan 2024",
        fontsize=12,
    )
    ...
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/citibike_end_binned_scatter.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved citibike_end_binned_scatter.png")
